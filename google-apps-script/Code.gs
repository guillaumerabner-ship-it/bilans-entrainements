...
function bridgeOutput_(data, origin, requestId) { const safeOrigin = /^https?:\/\/(localhost|127\.0\.0\.1)(:\d+)?$/.test(String(origin || '')) || /^https:\/\/[a-z0-9.-]+$/i.test(String(origin || '')) ? String(origin) : '*'; const message = JSON.stringify({ appTrainingBridge: true, requestId: String(requestId || ''), data: data }); return HtmlService.createHtmlOutput('<!doctype html><meta charset="utf-8"><script>parent.postMessage(' + JSON.stringify(message) + ',' + JSON.stringify(safeOrigin) + ');<\/script>').setXFrameOptionsMode(HtmlService.XFrameOptionsMode.ALLOWALL); }
