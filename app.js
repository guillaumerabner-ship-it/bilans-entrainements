...
const { buildSessionOverride, applySessionOverride, compactSessionOverride, effectiveSetValues, effectiveComment, normalizeSeriesCount, plannedSeriesCount, googleCredentialNeedsRefresh, exerciseMatchesSearch, reconcileSessionExercises, remapSessionProgress } = SessionSyncPriority;
...
function selectCoachStudent(student, result) {
  ...
  const videosByStatus = { pending: videos.filter((video) => !video.status || video.status === 'coach-review'), approved: videos.filter((video) => video.status === 'approved'), redo: videos.filter((video) => video.status === 'redo') };
  const filteredVideos = coachVideoFilter === 'all' ? videos : videosByStatus[coachVideoFilter] || []; const recentSessions = [...sessions].sort((a, b) => b.date.localeCompare(a.date)).slice(0, 5);
  document.querySelector('#coach-dashboard').innerHTML = `<div class="coach-overview"><article><span>SÉANCES PLANIFIÉES</span><strong>${sessions.length}</strong></article><article><span>SÉANCES TERMINÉES</span><strong>${completed}</strong></article><article><span>VIDÉOS À REVOIR</span><strong>${videosByStatus.pending.length}</strong></article></div><div class="coach-actions"><button type="button" data-coach-open-calendar>Voir le calendrier</button><button type="button" data-coach-new-session>Créer une séance</button><button type="button" data-coach-open-videos>Bibliothèque vidéo</button></div><section class="coach-section"><div class="coach-section-title"><span>PROCHAINES ET DERNIÈRES SÉANCES</span><b>${sessions.length}</b></div><div class="coach-session-list">${recentSessions.length ? recentSessions.map((session) => `<button type="button" data-coach-session="${escapeHtml(session.id)}"><time>${new Date(`${session.date}T12:00`).toLocaleDateString('fr-FR', { day: '2-digit', month: 'short' })}</time><span><strong>${escapeHtml(session.name)}</strong><small>${session.exercises?.length || 0} exercice(s)</small></span><b>${sessionCompletion(session).state === 'complete' ? '✓' : '→'}</b></button>`).join('') : '<p class="coach-empty-line">Aucune séance disponible.</p>'}</div></section><section class="coach-section"><div class="coach-section-title"><span>REVUE DES VIDÉOS</span><b>${filteredVideos.length}</b></div><div class="coach-video-filters"><button type="button" data-coach-video-filter="pending" class="${coachVideoFilter === 'pending' ? 'active' : ''}">À revoir <b>${videosByStatus.pending.length}</b></button><button type="button" data-coach-video-filter="approved" class="${coachVideoFilter === 'approved' ? 'active' : ''}">Validées <b>${videosByStatus.approved.length}</b></button><button type="button" data-coach-video-filter="redo" class="${coachVideoFilter === 'redo' ? 'active' : ''}">À refaire <b>${videosByStatus.redo.length}</b></button><button type="button" data-coach-video-filter="all" class="${coachVideoFilter === 'all' ? 'active' : ''}">Toutes <b>${videos.length}</b></button></div><div class="coach-video-review">${filteredVideos.length ? filteredVideos.map(coachVideoReviewCard).join('') : '<p class="coach-empty-line">Aucune vidéo dans ce filtre.</p>'}</div></section>`;
  if (sharedBackendReady() && currentGoogleCredential()) loadSharedSnapshot();
}
function videoReviewLabel(video) {
  if (video.status === 'approved') return { label: 'Vidéo validée', message: video.coachComment || 'Bravo, la vidéo est validée par ton coach.', tone: 'approved' };
  if (video.status === 'redo') return { label: 'À refaire', message: video.coachComment || 'Ton coach te demande de refaire cette vidéo.', tone: 'redo' };
  return { label: 'En attente du coach', message: '', tone: 'pending' };
}
function renderCoachActivity() {
  const feed = document.querySelector('#coach-activity-feed'); const count = document.querySelector('#coach-activity-count');
  if (!feed || !count) return;
  const reviewed = videoRecords().filter((video) => video.status === 'approved' || video.status === 'redo').sort((a, b) => String(b.reviewedAt || b.modifiedAt || '').localeCompare(String(a.reviewedAt || a.modifiedAt || '')));
  count.textContent = `${reviewed.length} retour${reviewed.length > 1 ? 's' : ''}`;
...
document.addEventListener('click', async (event) => {
  ...
  if (dialog?.open) { const session = getSessions().find((item) => item.id === dialog.dataset.sessionId); if (session) renderWorkoutVideoTools(session); }
});
...
function renderSessionExerciseOptions(selectedExercises = []) {
  ...
  document.querySelector('#session-exercise-options').innerHTML = getDisplayExercises().map((exercise) => {
    ...
    return `<label class="session-exercise-option" data-session-exercise-option="${escapeHtml(exercise.id)}"><input type="checkbox" data-session-exercise="${escapeHtml(exercise.id)}" ${checked ? 'checked' : ''} /><span><strong>${escapeHtml(exercise.name)}</strong><small>${exercise.metrics.map((metric) => labels[metric]).join(' · ')}</small></span><span class="series-choice">Séries <input type="number" min="1" max="20" value="${count}" data-series-for="${escapeHtml(exercise.id)}" ${checked ? '' : 'disabled'} /></span></label>`;
  }).join('');
}
function filterSessionExerciseOptions() {
  const query = document.querySelector('#session-exercise-search').value; const catalog = new Map(getDisplayExercises().map((exercise) => [exercise.id, exercise])); let visible = 0;
  document.querySelectorAll('[data-session-exercise-option]').forEach((option) => { const matches = exerciseMatchesSearch(catalog.get(option.dataset.sessionExerciseOption), query); option.hidden = !matches; if (matches) visible += 1; });
  const status = document.querySelector('#session-exercise-search-status'); status.textContent = query.trim() ? `${visible} exercice${visible > 1 ? 's' : ''} trouvé${visible > 1 ? 's' : ''}` : '';
}
function openSessionForm(session = null, options = {}) {
  ...
  document.querySelector('#session-exercise-search').value = ''; renderSessionExerciseOptions(session?.exercises || []); filterSessionExerciseOptions(); dialog.showModal();
}
document.querySelectorAll('[data-open-form]').forEach((button) => button.addEventListener('click', () => openSessionForm()));
document.querySelector('#session-exercise-search').addEventListener('input', filterSessionExerciseOptions);
...
function renderDashboard() {
  ...
  renderJournal(); renderBlockCockpit(block, sessions); renderWeeklyVolume(); renderTrophies(); renderCoachActivity();
...
document.querySelector('#workout-content').addEventListener('input', (event) => {
  ...
  if (event.target.matches('[data-workout-exercise-search]')) {
    const session = getSessions().find((item) => item.id === sessionId); let visible = 0;
    document.querySelectorAll('[data-workout-exercise-index]').forEach((section) => { const exercise = session?.exercises?.[Number(section.dataset.workoutExerciseIndex)]; const rawName = typeof exercise === 'string' ? exercise : exercise?.name; const searchable = resolveCatalogExercise(rawName) || exercise; const matches = exerciseMatchesSearch(searchable, event.target.value); section.hidden = !matches; if (matches) visible += 1; });
    document.querySelector('[data-workout-exercise-search-status]').textContent = event.target.value.trim() ? `${visible} exercice${visible > 1 ? 's' : ''} affiché${visible > 1 ? 's' : ''}` : ''; return;
  }
